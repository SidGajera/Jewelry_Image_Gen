...
Load and enforce **`docs/24_PERMANENT_ERROR_PREVENTION_LOCK.md`** (the consolidated standing lock: loads docs/03·04·11·12·13·14·16·21·22 + Failure Memory; source-jewelry lock · premium-cloth lock · official-logo lock · jewelry-first · failure memory · catalog execution · final delivery gate · response contract) and **`docs/22_PRE_GENERATION_GATE.md`** (source lock · jewelry-priority focus hierarchy · logo lock: black tagline + single instance + print-into-fabric · premium cloth benchmark · failure memory · generation gate) **plus the runtime trio** (`config/project_manifest.json`, `prompts/07_PROMPTS.md`, `config/QUALITY_MEMORY.json`). Do not generate until all are active.
