...
> **ARCHIVED 2026-07-16 — NOT AUTHORITATIVE. DO NOT LOAD.**
> Historical snapshot of the pre-split LUCENT_MASTER.md, kept for provenance only. Every rule here is owned elsewhere and may be STALE — notably the Drive map below lists the OLD source parent (`1mKqVAi2iv...`); the current one is in `config/project_manifest.json` / `docs/06_CACHE.md`.
> Authoritative owners: image-generation `docs/03` · logo `docs/04` · background `docs/11` · geometry `docs/13` · QC `docs/18` · prompts `prompts/07_PROMPTS.md` · runtime `CLAUDE_SETUP.md` · architecture + business rules `LUCENT_MASTER.md`.
