...
2. **LOGO second — the ONE exact logo on the cloth.** Reproduce the EXACT official logo (never garbled/approximated/a different design/duplicated); rendered IN-MODEL by Higgsfield (user-locked 2026-07-21; local composite retired) to look permanently printed INTO the cloth before the jewelry was photographed — following texture/weave/folds/perspective/lighting/shadows, BLACK tagline, no background wireframe. May be fully visible / partially cropped / partly hidden by folds; never a watermark/floating overlay/sticker/digital graphic. (docs/04 override, docs/22 §2)
...
**FINAL VALIDATION (every studio image):** ✓ approved premium white cloth used ✓ preserved official logo used ✓ logo naturally printed into cloth ✓ jewelry placed after background prep ✓ physics correct ✓ neutral white balance ✓ NO AI-generated logo ✓ NO AI-generated cloth. This sequence is mandatory for every office/studio photoshoot. (Practical pipeline 2026-07-21: the logo is rendered IN-MODEL by Higgsfield — local composite retired; the build ORDER and validation still govern the final deliverable, `docs/04` override.)
...
- The printed logo follows the same physically correct cloth surface: bends/warps naturally with folds and perspective; may be fully visible, partially hidden, or naturally cropped; never floats above the cloth or stays unnaturally flat across deep folds. Preserve the exact official artwork and color. (Rendered in-model by Higgsfield per `docs/04` override, 2026-07-21; held to this by strong prompting + QC + regenerate.)
