...
- **Position — CENTERED:** the ring is placed centered horizontally, centered vertically, and visually balanced in the frame. Never place it near the edges. (This replaces the earlier off-center/rule-of-thirds requirement.)
  - **NARROW CARVE-OUT — the editorial lifestyle slot only (user-locked 2026-07-20).** `20` §2.5 (Slot 5, Lifestyle / Proposal Scene) may place the ring in the left or right third. That frame's job is emotional and editorial, and forcing centre composition there pushes the props into symmetrical staging around the ring — the over-perfect arrangement that reads as AI-generated. **Centred stays mandatory on every other frame**, where buyers compare product images and consistency is worth more than character. The ring must still be the instant focal point, framed large, with the background never sharper than it.
