...
## PREVIEW OPTIMIZATION RULE (user-locked 2026-07-16)
Generate ONLY final, full-quality output. Never spend credits/tokens on throwaway artifacts.
- **Never generate:** preview images, draft renders, thumbnails, comparison collages, intermediate/"work in progress" outputs, or low-quality test images. Only generate the final requested image(s).
- **Never display intermediate processing results** or create a preview version "for confirmation."
- **Regeneration = replace, not accumulate:** if a shot must be redone, replace the previous image — never produce multiple preview versions of the same shot.
- **Exact count:** generate exactly the number of FINAL images requested — no extras.
