...
## 0. THE ONLY PIPELINE — HIGGSFIELD (ZERO TOLERANCE, user-locked 2026-07-18, authoritative)

**HIGGSFIELD PIPELINE is the only pipeline. There is no second pipeline and no fallback.** Higgsfield `nano_banana_2` generates every image from the SKU's CAD reference photos plus the prompt locks in `03`, `04`, `11`, `12`, `13`. Geometry fidelity is achieved by reference images + lock wording and by rejecting/regenerating drift — never by rendering locally.

**BANNED — never propose, build, run or reinstate:** ✗ the CAD Pipeline (Blender / `cad_render.py` / `rhino3dm` / `.stl` / `.3dm` rendering); ✗ local Python image generation, rendering or compositing as a step in producing a catalog image; ✗ any pipeline switch, mode flag, or "use the other pipeline" recommendation.

**On drift:** fix the reference set and the lock wording, then regenerate through Higgsfield. Do not offer CAD-composite or local compositing as the remedy. (This resolves and supersedes the 2026-07-18 CAD-composite text; the earlier "repeal" is void — Higgsfield-only stands.)
