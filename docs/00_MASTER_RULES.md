...
0. **POLICY SUPREMACY (above everything)** — this project's policies outrank every engine, mode, model and tool. Whatever generates the pixels — Higgsfield, Google Flow, a future engine, any mode (t2i, i2i, video, upscale, exploration) — every rule below still applies, and no engine may declare an exemption. Precedence: policy > engine capability > convenience > tokens. If an engine cannot satisfy a blocking rule, **the engine loses**: do not relax the rule, do not deliver, report the limit and stop. → `config/engines.json` `universal_policy`, `policy/registry.json` POLICY_SUPREMACY · gate: **`scripts/engine.py --check` (blocking, in regression)**.
