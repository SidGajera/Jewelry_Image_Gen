...
> **PACKAGING LOCK — PURE WHITE (user-locked 2026-07-20, revised same day).** The jewellery box and every packaging material are **locked brand assets**, exactly like the cloth (`11`) and the logo (`04`). Everything is **PURE WHITE — clean neutral white, never cream, ivory, beige, off-white or tan, with no warm cast**: pure white leatherette box with pure white ribbed grosgrain rim trim and a pure white velvet twin-cushion interior · pure white cotton pouch with white drawstring · the approved pure white logo cloth · a pure white blank card · neutral white surface. The gold of the ring is the only warm colour in frame. **The materials never change between images within a catalog or across catalogs.** (Master render: LR-0169 slot 10, job `25e2b3db`. The earlier cream/ivory master `f6bf9ae7` is SUPERSEDED — do not reference it.) **Never substitute** a navy, dark, velvet-shell, round or otherwise different box — not in slot 10, and not in any lifestyle frame that happens to include a box. **The box must be identical in every image within a catalog AND across catalogs.** Feed the master render in as a reference image on any frame containing the box; describing it in words is not sufficient. *Observed 2026-07-20 (LR-0169): slot 10 rendered the approved cream box while slot 5 was styled with a navy velvet box — two different boxes in one catalog.* Pool C's `C1 open navy velvet ring box` (§4) is therefore **retired**; where a box appears, it is always the approved cream box.
