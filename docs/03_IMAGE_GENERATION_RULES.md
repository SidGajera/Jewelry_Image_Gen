...
**NO TEXT / NO WATERMARK (user-locked 2026-07-18, all images).** Never render any text, caption, label, title, watermark, brand words, category words (e.g. "HOUSE LIFESTYLE"), sticker or graphic overlay onto ANY image. Lifestyle and close-up images carry NO logo and NO text at all. Studio images carry ONLY the official Lucent Carat Lab logo printed into the cloth (owner `04`) — nothing else. Reject immediately if any stray text, caption or watermark appears. Recurrence 2026-07-18: a lifestyle frame rendered the caption "HOUSE LIFESTYLE" over the scene.

**LIFESTYLE GEOMETRY LOCK (user-locked 2026-07-18).** For lifestyle images, NEVER reconstruct or redesign the jewelry. The source CAD is the ONLY geometry authority. An uploaded lifestyle/composition image is ONLY a composition reference — use it solely for hand pose · finger position · camera · lighting · environment · depth of field, NEVER as a jewelry reference. Reproduce the jewelry exclusively from the approved CAD. Before approving, compare against the CAD and reject immediately if ANY of these differ: band path · band thickness · twist location · shoulder geometry · gallery · basket · head · cathedral · pavé count · pavé spacing · prongs · diamond position · silhouette. One landmark off → reject and regenerate.
*Accepted approach (user-locked 2026-07-18, "use Higgsfield only"): Higgsfield is the only pipeline; the CAD render/composite path is NOT to be proposed as the remedy. With no locked 3D mesh the model may still drift slightly on twist-band geometry — the accepted remedy is best-effort reference locking + regenerate through Higgsfield until it passes the overlay check; do not offer CAD-composite.*
...
## C. METAL RENDERING
- Default **14K natural yellow gold** (per-listing parameter — see the 2026-07-20 rule below; the historic default was 18K). Warm yellow, premium polished finish. Never rose/white/silver/pale unless that is the listing's declared metal or the source is that metal.
...
### METAL IS A PER-LISTING PARAMETER (user-locked 2026-07-20 — supersedes the global 18K lock below)
The metal is **declared per listing**, not fixed globally. **Default: 14K natural yellow gold.** The declared metal is recorded in that SKU's `config/deliveries/LR-XXXX.json` and becomes the lock **for that listing** — every image in the catalog uses that one metal, with no variation in hue · warmth · saturation · brightness · reflectivity · finish, exactly as the consistency rules below require. Lighting still changes REFLECTIONS ONLY, never the base metal colour.

**Exception — the metal-comparison slot (`20` §2.9).** That one frame renders the metals the listing actually offers (e.g. 14K yellow · 14K white with rhodium · 14K rose), side by side at identical geometry and scale. **Skip the slot entirely for single-metal listings** — never show a metal that is not for sale.

Everything below stays in force with "18K yellow gold" read as "the listing's declared metal": the per-catalog consistency requirement, the reject-on-hue-shift bar, and the forbidden-appearance list (pale · champagne · orange · brass · bronze · dark · brown · greenish · over-saturated · washed-out · mixed tones) applied to whichever metal is declared.

**MASTER METAL COLOUR — LOCKED TO SOURCE (user-locked 2026-07-18; karat and colour now set per listing per the 2026-07-20 rule above).** The metal colour is locked to the approved source reference: **Premium Polished 18K Yellow Gold** — warm luxury gold, NEUTRAL yellow, rich but not orange, bright mirror polish, high reflectivity, natural metal reflections, consistent hue throughout the ring. Identical across office/lifestyle/close-up/macro/hand/side/rear/top and every future catalog. The AI must NEVER reinterpret the metal colour and never produce: pale yellow · white-yellow · champagne · rose tint · orange · brass · bronze · dark · brown · greenish · over-saturated yellow · washed-out · mixed gold tones. **Lighting may change REFLECTIONS ONLY — never the base metal colour** (not by camera angle, white balance, environment, background, exposure, HDR, cloth colour, indoor/outdoor light). Before approving, compare to prior approved images; reject on any hue/saturation/temperature/brightness shift or if the metal reads like another alloy.
