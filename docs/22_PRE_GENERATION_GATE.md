...
## 2. LOGO LOCK
**PRIMARY METHOD — LOGO IS PRINTED BY THE LOCAL COMPOSITE (user-locked 2026-07-21, LATEST).** Higgsfield renders the studio/packaging shot on **CLEAN premium-white cloth (no logo, no text, no background graphic)**; then `scripts/print_logo_on_cloth.py` prints the ONE exact preserved logo into the cloth (pixel-exact, approved params `--scale 0.28-0.34 --pos lower-right/lower-centre --opacity 0.45-0.6 --displace 6-8 --soften 1.0 --grain 0.06`). **In-model logo is NOT used** — it repeatedly produced oversized/duplicated/emblem-as-wireframe/overlay logos (`config/QUALITY_MEMORY.json#lr0180-inmodel-logo-overlay-and-duplicate`). REQUIRED result: exact preserved logo (gold emblem + "LUCENT CARAT LAB", **BLACK** "FUTURE OF FINE JEWELRY" tagline), EXACTLY ONE complete lockup, printed INTO the weave (texture through the ink, follows folds/light, no overlay/sticker/glow/box/independent-shadow), jewelry the sharp hero, logo soft/secondary. ENVIRONMENT: the composite runs where the render is downloadable; a session whose egress blocks the Higgsfield CDN/Drive (this web sandbox, 403) makes the clean-cloth base only and hands the composite to desktop/operator. The rules below define what the printed logo must satisfy.
