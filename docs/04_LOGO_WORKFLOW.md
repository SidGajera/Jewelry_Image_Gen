...
*Execution note: an in-model render always re-draws the logo, so this lock is only guaranteed by the local composite path — generate on clean cloth, then print the master asset with `scripts/print_logo_on_cloth.py` (§4 fallback, pixel-exact, 0 credits). Any in-model logo must be checked against the master and rejected on any difference.*
