...
Plus placement rules: natural, lower area of the frame (lower-centre / lower-right), never dead-centre under the ring. **COMPLETE & READABLE LOCK (user-locked 2026-07-21, supersedes the earlier "partial visibility preferred"):** the ONE logo must appear **complete and readable** — never cropped by the frame, never split, tagline and emblem never cut off or hidden. It may read naturally softer than the tack-sharp jewelry purely from optical depth-of-field, but is never blurred independently and never partially amputated. Place and scale it so the whole lockup sits inside the frame on the cloth. Logo pixels remain identical (only blended into the cloth lighting, never repainted).
