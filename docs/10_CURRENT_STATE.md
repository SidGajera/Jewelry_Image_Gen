...
- **7 × CLOSED HAND (worn) images** — the ring worn on a natural hand (five fingers, natural skin), closeup/lifestyle, cozy warm US-home, NO laptop/desk/office, NO logo. Vary pose/angle/background across the 7 (hand near face, back-of-hand, side-of-finger, on knit/marble/linen, etc.); ring tack-sharp, only background blurs; true manufactured size.
  ...
  **CLOSE-UP WEIGHTING (user-locked 2026-07-24, applies from the catalog AFTER LR-0189).** Within those 7, favour house-lifestyle CLOSE-UPS: **4 × macro/close-up + 3 × wider worn shots** (was 2 + 5). Close-ups are where the stone run, facet quality and setting detail actually read, so they carry more of the catalog. Draw close-up scenes from the `Closup_houselifestyle` reference set in `docs/06 §LIFESTYLE — CLOSE-UP REFERENCES`, and honour the pose-variety rule: different reference files than the previous SKU. Suggested close-up spread — top-down detail of the worn band · side-of-finger macro · held in fingertips · three-quarter macro across the stone run.
