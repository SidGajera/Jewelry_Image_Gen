...
- CLOTH (P3): the one locked premium matte white pouch cloth — same material/texture/weave/grain/softness every image; never silk/satin/linen/velvet/marble/paper/leather/other. Plain OR naturally folded (don't force folds). Pure NEUTRAL white always; never cream/ivory/beige/yellow/orange/pink/red/gray/blue or any tint/warm cast.
...
- LOGO (P0, GOVERNING = OMIT OVER APPROXIMATE): AI NEVER recreates/redraws/infers/approximates the logo. Use the exact preserved `assets/logo/logo_official.png` (local composite `scripts/print_logo_on_cloth.py`, or an in-model render only if verified pixel-identical) OR OMIT it. A MISSING logo is acceptable; an INCORRECT logo is NOT. Never AI text as branding. When present it must be pixel-identical (icon/typography/stars/lines/tagline/GOLD color/spacing/proportions) and read as metallic ink printed into the cloth (follows folds/perspective/light; may be cropped/partly hidden; never floats). Never the hero.
